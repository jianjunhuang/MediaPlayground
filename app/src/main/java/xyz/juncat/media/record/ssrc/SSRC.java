...
package xyz.juncat.media.record.ssrc;
...
public class SSRC {
    ...
    private static final String TAG = "SSRC";
...
//  private static final int M = 15;
    ...
    public int init_shaper(int freq, int nch, int min, int max, int dtype, int pdf, double noiseamp) {
        ...
        if ((dtype == 3 || dtype == 4) && i == 6) {
            Log.w(TAG, String.format("Warning: ATH based noise shaping for destination frequency %dHz is not available, using triangular dither\n", freq));
        }
        if (dtype == 2 || i == 6) {
            i = 0;
        }
     ...
     */
    private void usage() {
        Log.w(TAG, "http://shibatch.sourceforge.net/\n\n");
        Log.w(TAG, "usage: ssrc [<options>] <source wav file> <destination wav file>\n");
        Log.w(TAG, "options : --rate <sampling rate>     output sample rate\n");
        Log.w(TAG, "          --att <attenuation(dB)>    attenuate signal\n");
        Log.w(TAG, "          --bits <number of bits>    output quantization bit length\n");
        Log.w(TAG, "          --tmpfile <file name>      specify temporal file\n");
        Log.w(TAG, "          --twopass                  two pass processing to avoid clipping\n");
        Log.w(TAG, "          --normalize                normalize the wave file\n");
        Log.w(TAG, "          --quiet                    nothing displayed except error\n");
        Log.w(TAG, "          --dither [<type>]          dithering\n");
        Log.w(TAG, "                                       0 : no dither\n");
        Log.w(TAG, "                                       1 : no noise shaping\n");
        Log.w(TAG, "                                       2 : triangular spectral shape\n");
        Log.w(TAG, "                                       3 : ATH based noise shaping\n");
        Log.w(TAG, "                                       4 : less dither amplitude than type 3\n");
        Log.w(TAG, "          --pdf <type> [<amp>]       select p.d.f. of noise\n");
        Log.w(TAG, "                                       0 : rectangular\n");
        Log.w(TAG, "                                       1 : triangular\n");
        Log.w(TAG, "                                       2 : Gaussian\n");
        Log.w(TAG, "          --profile <type>           specify profile\n");
        Log.w(TAG, "                                       standard : the default quality\n");
        Log.w(TAG, "                                       fast     : fast, not so bad quality\n");
    }
    ...
    private void showprogress(double p) {
        ...
        if (pc != lastshowed2 || t != lastshowed) {
            Log.w(TAG, String.format(" %3d%% processed", pc));
            lastshowed2 = pc;
        }
        if (t != lastshowed) {
            Log.w(TAG, String.format(", ETA =%4dmsec", eta));
            lastshowed = t;
        }
        Log.w(TAG, "\r");
        System.err.flush();
...
//for(i=0;i<n2b2;i++) { Log.w(TAG,"%d:%g",i,buf2[ch][i]); }
...
//Log.w(TAG,"dfrq: %d\n", dfrq);
        ...
        if (!quiet) {
            Log.w(TAG, "Shibatch sampling rate converter version " + VERSION + "(high precision/nio)\n\n");
        }
...

        {
            ...
            while (true) {
                ...
                int c3 = bb.get();
                length = bb.getInt();
                Log.w(TAG, String.format("chunk: %c%c%c%c\n", c0, c1, c2, c3));
        ...
        if (!quiet) {
            ...
            Log.w(TAG, String.format("frequency : %d -> %d\n", sfrq, dfrq));
            Log.w(TAG, String.format("attenuation : %gdB\n", att));
            Log.w(TAG, String.format("bits per sample : %d -> %d\n", bps * 8, dbps * 8));
            Log.w(TAG, String.format("nchannels : %d\n", nch));
            Log.w(TAG, String.format("length : %d bytes, %g secs\n", length, (double) length / bps / nch / sfrq));
            if (dither == 0) {
                Log.w(TAG, String.format("dither type : none\n"));
            } else {
                Log.w(TAG, String.format("dither type : %s, %s p.d.f, amp = %g\n", dtype[dither], ptype[pdf], noiseamp));
            }
            Log.w(TAG, "\n");
        }
...
//Log.w(TAG,"nch: %d, bps: %d, size: %d, sfrq: %d, dfrq: %d, ???: %d, ???: %d, twopass: %b, dither: %d\n", nch, bps, 8, sfrq, dfrq, 1, length / bps / nch, twopass, dither);
            if (normalize) {
                if (sfrq < dfrq) {
...
//System.err.println("ft: " + ft);
                if (ft.delete() == false) {
                    Log.w(TAG, String.format("Failed to remove %s\n", ft));
                }
            }
        } else {
            if (sfrq < dfrq) {
        ...
        if (!twopass && peak[0] > 1) {
            if (!quiet) {
                Log.w(TAG, String.format("clipping detected : %gdB\n", 20 * Math.log10(peak[0])));
            }
        }
    ...
    protected byte[] getDataFromByteBuffer(ByteBuffer rawoutbuf) {
        ...
        Log.w(TAG, "getDataFromByteBuffer: " + tempDataWrt.length);
        return tempDataWrt;
    }
